...
class OuraParser(IngestionBase):
    ...
    def parse_directory(self, dir_path: str):
        ...
        readiness_df = self._read_csv_robust(
            os.path.join(dir_path, "dailyreadiness.csv")
        )
        ...
        day_stress_df = self._read_csv_robust(
            os.path.join(dir_path, "daytimestress.csv")
        )

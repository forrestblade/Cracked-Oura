...
class IngestionBase:
    ...
    def _read_csv_robust(self, file_path: str) -> Optional[pd.DataFrame]:
        ...
        if (
            "dailyactivity.csv" in file_path.lower()
            or "sleepmodel.csv" in file_path.lower()
        ):
    ...
    def _upsert(self, model: Type[Base], data: List[Any], index_elements: List[str]):
        ...
        try:
            ...
            update_dict = {
                col.name: col for col in stmt.excluded if col.name not in index_elements
            }
    ...
    def _parse_sequence_to_timestamped_list(
        self, val, start_time: datetime, interval_seconds: int
    ):
        """
            ...
            if items is None:
                ...
                if "," in val_cleaned:
                    try:
                        items = [
                            float(x.strip())
                            for x in val_cleaned.strip("[]").split(",")
                            if x.strip()
                        ]
                    except Exception:
                        pass
        ...
        for i, item in enumerate(items):
            ...
            if isinstance(item, dict):
                ...
                val_to_store = {"timestamp": ts.isoformat(), "value": item}

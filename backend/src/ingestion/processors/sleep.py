        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                sleep = SleepSession(
                    ...
                    total_sleep_duration=self._parse_int(
                        row.get("total_sleep_duration")
                    ),
                    ...
                    light_sleep_duration=self._parse_int(
                        row.get("light_sleep_duration")
                    ),
                    ...
                    sleep_phase_5_min=self._parse_sequence_to_timestamped_list(
                        row.get("sleep_phase_5_min"), bedtime_start, 300
                    ),
                    sleep_phase_30_sec=self._parse_sequence_to_timestamped_list(
                        row.get("sleep_phase_30_sec"), bedtime_start, 30
                    ),
                    movement_30_sec=self._parse_sequence_to_timestamped_list(
                        row.get("movement_30_sec"), bedtime_start, 30
                    ),
                    # Detailed fields
                    ...
                    low_battery_alert=(
                        bool(self._parse_int(row.get("low_battery_alert")))
                        if row.get("low_battery_alert")
                        else None
                    ),
                    ...
                    hr_data=self._parse_sequence_to_timestamped_list(
                        row.get("heart_rate"), bedtime_start, 300
                    ),
                    hrv_data=self._parse_sequence_to_timestamped_list(
                        row.get("hrv"), bedtime_start, 300
                    ),
                    readiness=self._parse_json_col(row.get("readiness")),
                    readiness_score_delta=self._parse_float(
                        row.get("readiness_score_delta")
                    ),
                )

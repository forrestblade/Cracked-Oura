    ...
    def process_readiness(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            rec = Readiness(
                ...
                temperature_deviation=self._parse_float(
                    row.get("temperature_deviation")
                ),
                temperature_trend_deviation=self._parse_float(
        ...
        for _, row in df.iterrows():
            try:
                ...
                rec = Resilience(
                    ...
                    sleep_recovery=(
                        float(contributors.get("sleep_recovery"))
                        if contributors
                        and contributors.get("sleep_recovery") is not None
                        else None
                    ),
                    daytime_recovery=(
                        float(contributors.get("daytime_recovery"))
                        if contributors
                        and contributors.get("daytime_recovery") is not None
                        else None
                    ),
                    stress=(
                        float(contributors.get("stress"))
                        if contributors and contributors.get("stress") is not None
                        else None
                    ),
                )
                records.append(rec)

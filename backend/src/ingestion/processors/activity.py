        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                rec = Activity(
                    ...
                    equivalent_walking_distance=self._parse_int(
                        row.get("equivalent_walking_distance")
                    ),
                    contributors=contributors,
                    # Converted sequences
                    class_5_min=self._parse_sequence_to_timestamped_list(
                        row.get("class_5_min"), day_start, 300
                    ),
                    met=self._parse_sequence_to_timestamped_list(
                        row.get("met"), day_start, 60
                    ),
                    # Detailed Stats
                    high_activity_met_minutes=self._parse_int(
                        row.get("high_activity_met_minutes")
                    ),
                    high_activity_time=self._parse_int(row.get("high_activity_time")),
                    inactivity_alerts=self._parse_int(row.get("inactivity_alerts")),
                    low_activity_met_minutes=self._parse_int(
                        row.get("low_activity_met_minutes")
                    ),
                    low_activity_time=self._parse_int(row.get("low_activity_time")),
                    medium_activity_met_minutes=self._parse_int(
                        row.get("medium_activity_met_minutes")
                    ),
                    medium_activity_time=self._parse_int(
                        row.get("medium_activity_time")
                    ),
                    ...
                    sedentary_met_minutes=self._parse_int(
                        row.get("sedentary_met_minutes")
                    ),
